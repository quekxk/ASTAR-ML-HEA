...
- If there are duplicate entries of the same composition (arising from different academic literature), the mean value of their mechanical properties are taken (provided none of the datapoints exceed 10% of the mean value).
...
### Data Scaling
Data scaling converts centers each input feature to have zero-mean and unit variance of 1. This has proven to allow faster model convergence when used in ML [^7].
...
The Mean Squared Error (MSE) represents the average of the squared differece between the original and predicted values in a dataset.
